...
func (m *Middleware) UserValidation(c *fiber.Ctx) error {
	...
	userData, err := jwt_tool.GetData[common.UserTokenData](accessToken, config.JWT_ACCESS_TOKEN_KEY)
	...
	if err != nil {
		...
		userData, err = jwt_tool.GetData[common.UserTokenData](refreshToken, config.JWT_REFRESH_TOKEN_KEY)
